...
router.get("/", passport.authenticate('jwt', { session: false }), (req, res) => {
    Profile.findOne({ user: req.user.id })
        .populate('user', ["name", "avatart"]) //填充user字段
        .then((profile) => {
            if (!profile) {
                return res.json({
                    success: false,
...
router.post("/", passport.authenticate('jwt', { session: false }), (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
        if (profile) {
            // 用户信息存在, 执行更新方法
            Profile.findOneAndUpdate({ user: req.user.id }, { $set: profileFields }, { new: true }).then(profile => res.json({
                success: true,
                code: 200,
                msg: "编辑成功",
                data: profile
    ...
    Profile.findOneAndRemove({ user: req.user.id })
        .then(() => {
            User.findOneAndRemove({ _id: req.user.id })
                .then(() => {
                    res.json({ success: true, code: 200, msg: "删除成功" })
                })
        })
})
